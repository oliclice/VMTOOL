...
if len(vac) == 2:
    ...
    def load_cache_settings(self):
        ...
        # 缓存过期时间
        cache_expiry_label = QLabel("缓存过期时间 (小时):")
        cache_expiry_combo = QComboBox()
        cache_expiry_combo.addItems(["1", "6", "12", "24", "48"])
        cache_expiry_combo.setCurrentText(str(config_manager.get("cache_expiry", 24)))
        # 连接信号，自动保存
        cache_expiry_combo.currentTextChanged.connect(lambda text: config_manager.set("cache_expiry", int(text)))
