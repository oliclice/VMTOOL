...
class Menu:
    ...
    def __init__(self, dict_manager: DictManager, weight_calc: WeightCalculator, file_writer: FileWriter):
    ...
    def _run_function(self, func_id: int):
        ...
        if func_id == 1:
            ...
            Timer.time_execution('计算权重', 
                                 lambda: self.weight_calc.calculate_all(self.dict_mgr.data))
        elif func_id == 3:
            Timer.time_execution('码表补充', self.dict_mgr.scan_and_add)
        elif func_id == 4:
            Timer.time_execution('写入码表', 
                                 lambda: self._write_and_copy())
        elif func_id == 5:
            ...
            Timer.time_execution('高频统计', 
                                 lambda: self.dict_mgr.count_high_keys(4, 10))
        elif func_id == 0:
            exit()
    ...
    def _write_and_copy(self):
        ...
        self.writer.write_dict(self.dict_mgr.data, self.dict_mgr.config.output_file, startwith_path)
        self.writer.copy_file(self.dict_mgr.config.output_file, 
                              self.dict_mgr.config.base_dir + '/dicts', 'main.txt')
        self.writer.copy_file(self.dict_mgr.config.output_file, 
                              self.dict_mgr.config.target_rime_path, 'flypy_user.txt')
...
class RichMenu:
    ...
    def __init__(self, dict_manager: DictManager, weight_calc: WeightCalculator, file_writer: FileWriter):
        ...
        self.weight_calc = weight_calc
        self.writer = file_writer
        self.menu = Menu(dict_manager, weight_calc, file_writer)
            ...
            def execute(input_str):
                ...
                for choice in re.split(r'[,\s]+', input_str.strip()):
                    ...
                    if func_id < len(self.dict_mgr.config.FUNCTIONS):
                        ...
                        start = datetime.now()
                        self.menu._run_function(func_id)
